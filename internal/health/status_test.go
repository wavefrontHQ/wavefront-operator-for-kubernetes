...
func TestReconcileReportHealthStatus(t *testing.T) {
	t.Run("report health status when all components are healthy", func(t *testing.T) {
		...
		componentsToCheck := map[string]string{
			...
			"wavefront-cluster-collector": "Deployment",
			"wavefront-node-collector":    "DaemonSet",
		}
		...
		proxyStatus := getComponentStatusWithName("wavefront-proxy", status.ComponentStatuses)
		assert.True(t, proxyStatus.Healthy)
		assert.Equal(t, "Running (1/1)", proxyStatus.Status)

		clusterCollectorStatus := getComponentStatusWithName("wavefront-cluster-collector", status.ComponentStatuses)
		assert.True(t, clusterCollectorStatus.Healthy)
		assert.Equal(t, "Running (1/1)", clusterCollectorStatus.Status)

		nodeCollectorStatus := getComponentStatusWithName("wavefront-node-collector", status.ComponentStatuses)
