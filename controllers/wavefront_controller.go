...
func (r *WavefrontReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if errors.IsNotFound(err) {
		err = withFSResources(r.FS, wavefront.Spec, r.KubernetesManager.deleteKubernetesObjects)
...
func (r *WavefrontReconciler) readAndCreateResources(spec wf.WavefrontSpec, resources []string) error {
	...
	err = r.KubernetesManager.createKubernetesObjects(resources, spec)
	if err != nil {
		return err
	}
	return nil
...
func (km KubernetesManager) deleteKubernetesObjects(spec wf.WavefrontSpec, resources []string) error {
	var resourceDecoder = yaml.NewDecodingSerializer(unstructured.UnstructuredJSONScheme)
	for _, resource := range resources {
		object := &unstructured.Unstructured{}
		_, gvk, err := resourceDecoder.Decode([]byte(resource), nil, object)
